...
import {ChemistryDefaultValues, ChemistryTypeFields} from "@src/@types/SystemTable";
import {ChemistrySchema} from "@src/schema/SystemTable";
...
const ChemistryTableForm = ({
  ...
  const methods = useForm({
    defaultValues: ChemistryDefaultValues,
  ...
  const {mutate: addTable} = useAddTableMutation(
    "/api/vet_clinic/chemistry/addChemistry",
  ...
  const {data: TableData} = useGetTableById(
    String(userId),
    "/api/vet_clinic/chemistry/getChemistryById?id=1",
  );

  const submit = (data: ChemistryTypeFields) => {
    const payload = {
      ...data,
    };
    const editPayload = {
...
export default ChemistryTableForm;
